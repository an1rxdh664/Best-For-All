...
from urllib.parse import quote_plus  # NEW: for Google Maps links
...
class GeoFoodSession:
    ...
    def _handle_new_search(self, query: str) -> str:
        ...
        lines.append(
            ...
            "- 'How to reach there / Google Maps link?'\n"
    ...
    def _parse_place_index_from_text(self, text_l: str) -> Optional[int]:
        """Understand 'first place', 'second place', 'third place'."""
        if "first place" in text_l or "1st place" in text_l:
            return 0
        if "second place" in text_l or "2nd place" in text_l:
            return 1
        if "third place" in text_l or "3rd place" in text_l:
            return 2
        # You can extend to 4th/5th if you want
        return None

    def _extract_phone_from_place(self, place: Dict[str, Any]) -> Optional[str]:
        """Try to get a phone number from raw geo place."""
        if not place:
            return None

        # Direct top-level field (if Phase_2 ever adds it)
        ...
        # Sometimes nested 'details' may exist
        details = place.get("details") or {}
        phone = details.get("phone") or details.get("formatted_phone_number")
        if phone:
            return str(phone).strip()

        return None

    def _build_maps_link(self, card: Dict[str, Any], place: Dict[str, Any]) -> Optional[str]:
        """Return a Google Maps link if possible."""
        if not card and not place:
            return None

        # 1. Prefer an existing Google Maps / maps.google.com link
        for candidate in [
            (card or {}).get("primary_link"),
            (place or {}).get("source_url"),
            (place or {}).get("website"),
        ]:
            if not candidate:
                continue
            url_l = str(candidate).lower()
            if "google.com/maps" in url_l or "maps.google.com" in url_l or "goo.gl/maps" in url_l:
                return str(candidate)

        # 2. Build a generic Maps search link from lat/lon (+ name)
        lat = (place or {}).get("lat")
        lon = (place or {}).get("lon")
        name = (card or {}).get("name") or (place or {}).get("name")
        if lat is not None and lon is not None:
            if name:
                q = quote_plus(f"{name} {lat},{lon}")
            else:
                q = quote_plus(f"{lat},{lon}")
            return f"https://www.google.com/maps/search/?api=1&query={q}"

        # 3. As a final fallback, just return primary_link/source_url even if not Maps
        for candidate in [
            (card or {}).get("primary_link"),
            (place or {}).get("source_url"),
            (place or {}).get("website"),
    ...
    def _handle_follow_up(self, text: str) -> str:
        ...
        idx: Optional[int] = self._parse_place_index_from_text(text_l)
        if idx is not None and ("detail" in text_l or "show" in text_l):
        ...
        wants_phone = any(m in text_l for m in phone_markers) or (
            ("phone" in text_l or "contact" in text_l) and "number" in text_l
        )
        if wants_phone:
            idx = self._parse_place_index_from_text(text_l)
            if idx is None:
                idx = self._get_focus_index_or_default()
            card, place = self._get_card_and_place_by_index(idx)
            if not card or not place:
            ...
            name = card["name"]
            phone = self._extract_phone_from_place(place)
            if phone:
                return f"The contact number for **{name}** is: {phone}"
            else:
                fallback_link = card.get("primary_link") or place.get("source_url") or place.get("website")
                if fallback_link:
                    return (
                        f"I couldn't find a direct phone number for **{name}** in the data.\n"
                        f"You can check this page (often shows phone/contact): {fallback_link}"
                    )
                return f"I couldn't find a phone number for **{name}** in the data."

        # 9) Directions / how to reach / Google Maps
        direction_markers = [
            "how to reach",
            "how do i reach",
            "how can i reach",
        ...
        if any(m in text_l for m in direction_markers):
            idx = self._parse_place_index_from_text(text_l)
            if idx is None:
                idx = self._get_focus_index_or_default()
            card, place = self._get_card_and_place_by_index(idx)
            ...
            name = card["name"]
            maps_link = self._build_maps_link(card, place)
            dist_disp = place.get("distance_display") or card.get("distance_display")

            if maps_link:
                extra = f" It's about {dist_disp} away." if dist_disp else ""
                return (
                    f"To reach **{name}**{extra}\n"
                    f"open this link in Google Maps:\n{maps_link}"
                )
            else:
                return (
                    f"I couldn't build a proper Google Maps link for **{name}**.\n"
                    "Try searching the name directly in Google Maps."
                )

        # 10) Generic "details" (no index)
        ...
        # 11) Fallback: recap the last summary
        return (
